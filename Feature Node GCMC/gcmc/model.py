...
class RecommenderSideInfoGAE(Model):
    ...
    def predict_all(self):
        """
        Compute full score matrix for Top-K recommendation.
        Returns:
            A tensor of shape [num_users, num_items] representing predicted scores.
        """
        return tf.matmul(self.user_embeddings, self.item_embeddings, transpose_b=True)
    ...
    def build(self):
        ...
        self.user_embeddings = input_u  # shape [num_users, hidden_dim]
        self.item_embeddings = input_v  # shape [num_items, hidden_dim]


# ---------------------- NEW: Feature-node model ----------------------

class RecommenderFeatNodeGAE(Model):
    def __init__(self, placeholders, input_dim_u, input_dim_v, input_dim_f,
                 num_classes, num_support_uv,
                 learning_rate, num_basis_functions, hidden, num_users, num_items,
                 self_connections=False, **kwargs):
        super(RecommenderFeatNodeGAE, self).__init__(**kwargs)

        # Inputs
        self.inputs = (placeholders['u_features'], placeholders['v_features'], placeholders['f_features'])
        self.u_features_nonzero = placeholders['u_features_nonzero']
        self.v_features_nonzero = placeholders['v_features_nonzero']
        self.f_features_nonzero = placeholders['f_features_nonzero']

        self.support_uv = placeholders['support']      # stacked rating supports (u<-v)
        self.support_vu = placeholders['support_t']    # stacked rating supports (v<-u)
        self.support_uf = placeholders['support_uf']   # U <- F
        self.support_fu = placeholders['support_fu']   # F <- U
        self.support_vf = placeholders['support_vf']   # V <- F
        self.support_fv = placeholders['support_fv']   # F <- V
        ...
        self.num_basis_functions = num_basis_functions
        self.num_classes = num_classes
        self.num_support_uv = num_support_uv
        ...
        # embeddings just before decoder
        # embeddings 就是 decoder 前的 user/item 表示
        self.embeddings = (self.user_embeddings, self.item_embeddings)
    ...
    def _build(self):
        # 1) Tripartite GCN
        self.layers.append(HeteroTripartiteGCN(
            input_dim_u=self.input_dim_u,
            input_dim_v=self.input_dim_v,
            input_dim_f=self.input_dim_f,
            output_dim=self.hidden[0],
            support_uv=self.support_uv,
            support_vu=self.support_vu,
            num_support_uv=self.num_support_uv,
            support_uf=self.support_uf,
            support_fu=self.support_fu,
            support_vf=self.support_vf,
            support_fv=self.support_fv,
            u_features_nonzero=self.u_features_nonzero,
            v_features_nonzero=self.v_features_nonzero,
            f_features_nonzero=self.f_features_nonzero,
            sparse_inputs=True,
            act=tf.nn.relu,
            dropout=self.dropout,
            logging=self.logging,
    ...
    def build(self):
        ...
        # First layer returns (u,v,f)
        gcn_u, gcn_v, gcn_f = self.layers[0](self.inputs)

        # Projection
        proj_u, proj_v = self.layers[1]([gcn_u, gcn_v])
        self.activations.append((proj_u, proj_v))

        # Decoder
        outputs = self.layers[2](self.activations[-1])
        ...
        self.user_embeddings = proj_u
        self.item_embeddings = proj_v
