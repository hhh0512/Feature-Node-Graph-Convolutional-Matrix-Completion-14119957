...
from gcmc.model import RecommenderGAE, RecommenderSideInfoGAE, RecommenderFeatNodeGAE
from gcmc.utils import construct_feed_dict, construct_feed_dict_featnodes
...
# Feature nodes switch: use side info as nodes in GCN
ap.add_argument('--feature_nodes', '-F', '-fn', dest='feature_nodes', action='store_true', help='Use feature nodes (U↔F, V↔F) inside GCN')
ap.set_defaults(feature_nodes=False)
...
if DATASET == 'flixster' or DATASET == 'douban' or DATASET == 'yahoo_music':
    ...
    u_features, v_features, adj_train, train_labels, train_u_indices, train_v_indices, \
        val_labels, val_u_indices, val_v_indices, test_labels, \
        test_u_indices, test_v_indices, class_values, v_dict = load_official_trainvaltest_split(DATASET, TESTING)
else:
...
if not FEATURES:
    ...
    # === Build feature-node supports if FEATURE_NODES is enabled ===
    if FEATURE_NODES:
        uf = sp.csr_matrix(normalize_features(u_features))  # U x F_u
        vf = sp.csr_matrix(normalize_features(v_features))  # V x F_v

        from scipy.sparse import hstack
        F_u = uf.shape[1]
        F_v = vf.shape[1]
        support_uf_full = hstack([uf, sp.csr_matrix((uf.shape[0], F_v), dtype=uf.dtype)], format='csr')
        support_vf_full = hstack([sp.csr_matrix((vf.shape[0], F_u), dtype=vf.dtype), vf], format='csr')

        support_uf_full = globally_normalize_bipartite_adjacency([support_uf_full], symmetric=SYM)[0]
        support_vf_full = globally_normalize_bipartite_adjacency([support_vf_full], symmetric=SYM)[0]
        support_fu_full = support_uf_full.T
        support_fv_full = support_vf_full.T

        num_feat_nodes = support_uf_full.shape[1]
        f_features_csr = sp.identity(num_feat_nodes, format='csr')
    else:
        support_uf_full = None
        support_vf_full = None
        support_fu_full = None
        support_fv_full = None
        f_features_csr = None
...
placeholders = {
    ...
    'f_features': tf.compat.v1.sparse_placeholder(tf.float32, shape=(None, None)),
    'f_features_nonzero': tf.compat.v1.placeholder(tf.int32, shape=()),
    'support_uf': tf.compat.v1.sparse_placeholder(tf.float32, shape=(None, None)),
    'support_fu': tf.compat.v1.sparse_placeholder(tf.float32, shape=(None, None)),
    'support_vf': tf.compat.v1.sparse_placeholder(tf.float32, shape=(None, None)),
    'support_fv': tf.compat.v1.sparse_placeholder(tf.float32, shape=(None, None)),
}


# create model
if FEATURES and FEATURE_NODES:
    model = RecommenderFeatNodeGAE(placeholders,
                                   input_dim_u=u_features.shape[1],
                                   input_dim_v=v_features.shape[1],
                                   input_dim_f=(f_features_csr.shape[1] if FEATURE_NODES else 0),
                                   num_classes=NUMCLASSES,
                                   num_support_uv=num_support,
...
if FEATURE_NODES:
    # 先按 train/val/test 切分（保证这些变量已存在）
    test_support_uf = support_uf_full[np.array(test_u), :]
    test_support_vf = support_vf_full[np.array(test_v), :]
    test_support_fu = support_fu_full[:, np.array(test_u)]
    test_support_fv = support_fv_full[:, np.array(test_v)]

    val_support_uf  = support_uf_full[np.array(val_u), :]
    val_support_vf  = support_vf_full[np.array(val_v), :]
    val_support_fu  = support_fu_full[:, np.array(val_u)]
    val_support_fv  = support_fv_full[:, np.array(val_v)]

    train_support_uf = support_uf_full[np.array(train_u), :]
    train_support_vf = support_vf_full[np.array(train_v), :]
    train_support_fu = support_fu_full[:, np.array(train_u)]
    train_support_fv = support_fv_full[:, np.array(train_v)]

    # 再做稀疏 -> tuple
    test_support_uf  = sparse_to_tuple(test_support_uf)
    test_support_vf  = sparse_to_tuple(test_support_vf)
    test_support_fu  = sparse_to_tuple(test_support_fu)
    test_support_fv  = sparse_to_tuple(test_support_fv)

    val_support_uf   = sparse_to_tuple(val_support_uf)
    val_support_vf   = sparse_to_tuple(val_support_vf)
    val_support_fu   = sparse_to_tuple(val_support_fu)
    val_support_fv   = sparse_to_tuple(val_support_fv)

    train_support_uf = sparse_to_tuple(train_support_uf)
    train_support_vf = sparse_to_tuple(train_support_vf)
    train_support_fu = sparse_to_tuple(train_support_fu)
    train_support_fv = sparse_to_tuple(train_support_fv)

    # F 节点的 one-hot 特征
    f_features = sparse_to_tuple(f_features_csr)
    f_features_nonzero = f_features[1].shape[0]
else:
    f_features = None
    f_features_nonzero = 0
...
if FEATURE_NODES:
    train_feed_dict = construct_feed_dict_featnodes(train_feed_dict, placeholders,
                                                    f_features, f_features_nonzero,
                                                    train_support_uf, train_support_fu,
                                                    train_support_vf, train_support_fv)
...
if FEATURE_NODES:
    val_feed_dict = construct_feed_dict_featnodes(val_feed_dict, placeholders,
                                                  f_features, f_features_nonzero,
                                                  val_support_uf, val_support_fu,
                                                  val_support_vf, val_support_fv)
...
if FEATURE_NODES:
    test_feed_dict = construct_feed_dict_featnodes(test_feed_dict, placeholders,
                                                   f_features, f_features_nonzero,
                                                   test_support_uf, test_support_fu,
                                                   test_support_vf, test_support_fv)
