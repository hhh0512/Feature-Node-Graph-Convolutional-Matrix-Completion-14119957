...
def map_data(data):
    ...
    data = np.asarray(list(data)).ravel()

 
    uniq, inv = np.unique(data, return_inverse=True)  

    id_dict = {val: idx for idx, val in enumerate(uniq)}
    n = len(uniq)

    return inv.astype(np.int64), id_dict, n
...
def load_data(fname, seed=1234, verbose=True):
    ...
    if fname == 'ml_100k':
        ...
        u_nodes_raw = data_array[:, 0].astype(dtypes['u_nodes'])
        v_nodes_raw = data_array[:, 1].astype(dtypes['v_nodes'])
        ratings     = data_array[:, 2].astype(dtypes['ratings'])

        
        u_nodes_ratings, u_dict, num_users = map_data(u_nodes_raw)
        v_nodes_ratings, v_dict, num_items = map_data(v_nodes_raw)
        ...
        movies_df = pd.read_csv(
            movies_file,
            sep=sep, header=None, names=movies_headers,
            engine='python',            
            encoding='latin-1'          
        )
    ...
    return num_users, num_items, u_nodes_ratings, v_nodes_ratings, ratings, u_features, v_features, v_dict

def convert_topk_to_titles(topk_items, v_dict, id2title, top_k=5):
    """
    将模型输出的 top-K 推荐结果中的内部索引，映射回原始 movie id 以及 movie title。

    Parameters
    ----------
    topk_items : list of list
        每个用户的 top-K 推荐，元素是内部 item 索引
    v_dict : dict
        由原始 item id → 内部索引 的映射
    id2title : dict
        由原始 item id → 电影标题 的映射
    top_k : int
        每个用户输出几个推荐结果
    ...
    -------
    result : list of dict
        每个用户对应一个字典，包含原始 item id 及标题
    """

    index2id = {v: k for k, v in v_dict.items()}
    result = []

    for user_idx, item_list in enumerate(topk_items):
        top_items_info = []
        for internal_id in item_list[:top_k]:
            raw_id = index2id.get(internal_id, None)
            if raw_id is None:
                continue
            title = id2title.get(raw_id, f"[Missing Title for ID {raw_id}]")
            top_items_info.append({
                'item_id': raw_id,
                'title': title
            })
        result.append({'user': user_idx, 'recommendations': top_items_info})
    
    return result


def build_v_dict_from_raw_file(dataset='ml_100k', seed=1234):
    """
    从原始 movielens 文件手动构建 v_dict
    """
    ...
    data = pd.read_csv(
        filename, sep=sep, header=None,
        names=['u_nodes', 'v_nodes', 'ratings', 'timestamp'], dtype={'v_nodes': np.int32})

    v_nodes = data['v_nodes'].values
    uniq = list(set(v_nodes))
