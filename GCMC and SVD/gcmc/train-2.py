...
if TESTING:
    ...
    pred_scores = sess.run(model.outputs, feed_dict=test_feed_dict)
    top_k = 5
    topk_items = np.argsort(-pred_scores, axis=1)[:, :top_k]

    print("\nTop-5 Recommendations for each user (first 10 users):")
    for user_id, items in enumerate(topk_items[:10]):
