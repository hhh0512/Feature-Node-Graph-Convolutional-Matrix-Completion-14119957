...
def create_trainvaltest_split(dataset, seed=1234, testing=False, datasplit_path=None, datasplit_from_file=False,
    ...
    if datasplit_from_file and os.path.isfile(datasplit_path):
        ...
        num_users, num_items, u_nodes, v_nodes, ratings, u_features, v_features, v_dict = load_data(dataset, seed=seed,
                                                                                            verbose=verbose)

        with open(datasplit_path, 'wb') as f:     
          pkl.dump([num_users, num_items, u_nodes, v_nodes, ratings, u_features, v_features],
    ...
    return u_features, v_features, rating_mx_train, train_labels, u_train_idx, v_train_idx, \
        val_labels, u_val_idx, v_val_idx, test_labels, u_test_idx, v_test_idx, class_values, v_dict
...
def load_official_trainvaltest_split(dataset, testing=False):
    ...
    u_nodes_ratings = np.array(list(u_nodes_ratings.item()), dtype=np.int64)
    v_nodes_ratings = np.array(list(v_nodes_ratings.item()), dtype=np.int32)
    ratings = ratings.astype(np.float64)
