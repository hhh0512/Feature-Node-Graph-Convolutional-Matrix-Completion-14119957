...
if DATASET == 'flixster' or DATASET == 'douban' or DATASET == 'yahoo_music':
    ...
    u_features, v_features, adj_train, train_labels, train_u_indices, train_v_indices, \
        val_labels, val_u_indices, val_v_indices, test_labels, \
        test_u_indices, test_v_indices, class_values, v_dict = create_trainvaltest_split(DATASET, DATASEED, TESTING,
...
if TESTING:
    ...
    # 加载电影ID到电影标题的映射
    id2title = {}
    with open('/content/drive/MyDrive/master project/data/ml_100k/u.item', encoding='ISO-8859-1') as f:
        for line in f:
            parts = line.strip().split('|')
            movie_id = int(parts[0])  # MovieLens 的原始ID是从1开始的
            movie_title = parts[1]
            id2title[movie_id] = movie_title
    
    print(f"Title for item 1377: {id2title.get(1377)}")
    print(f"Title for item 1274: {id2title.get(1274)}")

    top_k = 5
    # 预测分数 & topk 排序（预测矩阵维度为 [num_users, num_items]）
    pred_scores = sess.run(model.predict_all(), feed_dict=test_feed_dict)
    topk_items = np.argsort(-pred_scores, axis=1)[:, :top_k]

    # 构造原始 item ID 到内部索引的反向映射
    reverse_v_dict = {v: k for k, v in v_dict.items()}  # v_dict: 原始ID -> idx
    print("🔁 reverse_v_dict sample:", list(reverse_v_dict.items())[:10])

    missing_count = 0
    for k in reverse_v_dict.values():
        if k not in id2title:
            print(f"Missing item: {k}")
            missing_count += 1
    print(f"Missing {missing_count} items in id2title")

    # ✅✅✅ 第一步：构造每个用户在训练集中看过的 item（使用内部索引）
    from collections import defaultdict
    train_user_seen = defaultdict(set)
    for u, i in zip(train_u_indices, train_v_indices):
        train_user_seen[u].add(i)

    # ✅✅✅ 第二步：过滤掉用户已经看过的 item
    filtered_topk_items = []
    for user_id, item_row in enumerate(topk_items):
        filtered = [i for i in item_row if i not in train_user_seen[user_id]]
        filtered_topk_items.append(filtered[:top_k])  # 最多 top_k 个

    # 将 topk 的 item 内部索引还原为原始 ID
    topk_items_raw = [
        [reverse_v_dict[i] for i in user_row if i in reverse_v_dict]
        for user_row in filtered_topk_items
    ]

    from gcmc.data_utils import convert_topk_to_titles
    # 打印检查
    print("pred_scores:", pred_scores.shape)
    print("Topk shape:", topk_items.shape)
    print("User 0 top items (original IDs):", topk_items_raw[0])
    print("Length of v_dict:", len(v_dict))
    print("Sample v_dict:", list(v_dict.items())[:5])

    # 获取推荐结果
    results = convert_topk_to_titles(topk_items_raw, v_dict, id2title)
    print("Results example:")
    for r in results[:10]:
        print(f"User {r['user']} has {len(r['recommendations'])} new recommendations:")
        for title in r['recommendations']:
            print(f"  - {title}")

    import json

    # 原始 results 应该类似这样（含 item_id 为 np.int32 类型）：
    # results = [{'user': 0, 'recommendations': [{'item_id': np.int32(1378), 'title': 'Movie A'}, ...]}, ...]

    # 将 np.int32 转为 int，确保可以写入 JSON
    cleaned_results = []
    for r in results:
        cleaned_r = {
            'user': int(r['user']),
            'recommendations': [{'item_id': int(item['item_id']), 'title': item['title']} for item in r['recommendations']]
        }
